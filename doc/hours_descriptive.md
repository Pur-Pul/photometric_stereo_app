...
|18.20.2025|    1h|Fixed a bug which cause the editor to be to large. Opened up the normal map list to non logged in users. Opened up front page to non logged users. Optimized the Viewer3D component. Fixed a bug which prevented textures from applying on Viewer3D component. Fixed a bug which prevented login from redirecting to front page. Fixed a bug which prevented photometric stereo function from starting.|
|     Total|206.5h||
