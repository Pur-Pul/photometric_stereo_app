...
describe('Upload options renders.', () => {
    test('Upload name field is visible.', () => {
        render(<Provider store={store}><UploadOptions normalMap={{ src: 'test' }} name={'test'} setName={() => {}} setReady={() => {}} /></Provider>)
    ...
    test('set ready is called with true by default.', () => {
        ...
        render(<Provider store={store}><UploadOptions normalMap={{ src: 'test' }} name={'test'} setName={() => {}} setReady={mockedSetReady} /></Provider>)
        expect(mockedSetReady.mock.calls.at(-1)[0]).toBe(true)
    })
    test('set ready is called with false if name is empty.', () => {
        const mockedSetReady = vi.fn()
        render(<Provider store={store}><UploadOptions normalMap={{ src: 'test' }} name={''} setName={() => {}} setReady={mockedSetReady} /></Provider>)
...
describe('Upload options is functional.', () => {
    test('Typing in name field calls setName', async () => {
        ...
        render(<Provider store={store}><UploadOptions normalMap={{ src: 'test' }} name={'test'} setName={mockedSetName} setReady={() => {}} /></Provider>)
